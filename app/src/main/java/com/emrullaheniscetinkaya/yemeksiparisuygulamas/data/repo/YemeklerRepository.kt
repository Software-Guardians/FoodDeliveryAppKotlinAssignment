...
class YemeklerRepository(var yemekDatasource: YemekDatasource) {
    suspend fun tumYemeklerGetir(): List<Yemekler> =yemekDatasource.tumYemeklerGetir()
    suspend fun sepetYemekGetir(kullanici_adi: String): List<SepetYemekler> = yemekDatasource.sepetYemekGetir(kullanici_adi)
